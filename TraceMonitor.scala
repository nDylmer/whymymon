...
/*
  prop file : Forall f . (close(f) -> @ Exists m . (!close(f) S open(f,m))) 
*/
...
class Formula_file(monitor: Monitor) extends Formula(monitor) {
  ...
  override def evaluate(): Boolean = {
    ...
    // assignments5 (main formula excluding leaf nodes):
      now(6) = now(7).not()
      now(5) = now(8).or(now(6).and(pre(5)))
      now(4) = now(5).exist(var_m.quantvar)
      now(3) = pre(4)
  ...
  val var_f :: var_m :: Nil = declareVariables(("f",false), ("m",false))(0)
  ...
  txt = Array(
    "Forall f . (close(f) -> @ Exists m . (!close(f) S open(f,m)))",
      "close(f) -> @ Exists m . (!close(f) S open(f,m))",
      "close(f)",
      "@ Exists m . (!close(f) S open(f,m))",
      "Exists m . (!close(f) S open(f,m))",
      "!close(f) S open(f,m)",
      "!close(f)",
      "close(f)",
